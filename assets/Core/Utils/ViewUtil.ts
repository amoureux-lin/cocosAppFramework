...
export class ViewUtil {
     ...
     */
    static createPrefabNode(path: string, bundleName: string = resManager.defaultBundleName): Node {
        const p = resManager.get(path, Prefab, bundleName);
        if (p) {
     ...
     * @param bundleName  资源包名
     */
    static createPrefabNodeAsync(path: string, bundleName: string = resManager.defaultBundleName): Promise<Node> {
        return new Promise(async (resolve, reject) => {
            const p = await resManager.loadAsync(bundleName, path, Prefab);
            if (p) {
    ...
    static addNodeAnimation(path: string, node: Node, onlyOne: boolean = true, isDefaultClip: boolean = false) {
        ...
        const clip = resManager.get(path, AnimationClip) as AnimationClip;
        if (!clip) {
            return;
        }
