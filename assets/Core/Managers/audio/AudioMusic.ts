...
export class AudioMusic extends AudioSource {
     ...
     */
    async load(url: string, callback?: Function, bundleName: string = resManager.defaultBundleName) {
        // 下一个加载的背景音乐资源
        if (this._isLoading) {
        ...
        var data: AudioClip = await resManager.loadAsync(bundleName, url, AudioClip);
        if (data) {
