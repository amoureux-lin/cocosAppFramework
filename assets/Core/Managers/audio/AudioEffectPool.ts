...
export class AudioEffectPool {
     ...
     * @returns 
     */
    async load(url: string | AudioClip, bundleName: string = resManager.defaultBundleName, onPlayComplete?: Function): Promise<number> {
        return new Promise(async (resolve, reject) => {
            ...
            if (url instanceof AudioClip) {
                ...
                clip = resManager.get(url, AudioClip, bundleName)!;
                if (!clip) {
                    this.res.set(bundleName, url);
                    clip = await resManager.loadAsync(bundleName, url, AudioClip);
                }
            }
     ...
     * @param bundleName    资源包名
     */
    put(aeid: number, url: string | AudioClip, bundleName: string = resManager.defaultBundleName) {
        let key: string;
        if (url instanceof AudioClip) {
            key = url.uuid;
        }
    ...
    release() {
        ...
        this.res.forEach((url: string, bundleName: string) => {
            resManager.release(bundleName, url);
        });
