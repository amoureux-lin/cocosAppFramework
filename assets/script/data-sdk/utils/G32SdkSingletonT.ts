...
export class SingletonT<TYPE extends ISingleton> {
    ...
    destory<T extends TYPE>(typeOrkey?: SingletonClass<T> | string) {
        if (typeOrkey) {
            ...
            this._datas.forEach(v => {
                if (v.isResident) {
                    G32Log.d(`${v.module}为常驻单列，不做销毁处理`);
                } else {
                    G32Log.d(`${v.module}销毁`);
                    v.destory && v.destory();
    ...
    debug() {
        G32Log.d(`************************** ${this.module} 开始 **************************`);
        this._datas.forEach((data, key, source) => {
            if ( data.debug ){
                data.debug();
            }else{
                G32Log.d(`${data.module} : 未实现debug接口`);
            }
        });
        G32Log.d(`************************** ${this.module} 结束 **************************`);
    }
