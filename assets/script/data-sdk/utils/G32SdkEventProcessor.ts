...
import { G32SdkDispatcher } from "./G32SdkDispatcher";
import { G32SdkSingleton } from "./G32SdkSingleton";
...
export class EventProcessor implements IEventProcessor {
     ...
     * @param eventName 
     * @param func -- 默认this.onDReceive
     */
    onD(eventName: string[]|string, func?: EventCallback) {
        let events:string[];
    ...
    on(args: EventAgrs): void {
        switch (args.bind) {
            ...
            default: G32Log.e(`on ${args.bind} 未知事件类型`)
        }
    }
    once(args: EventAgrs): void {
        args.once = true;
    ...
    off(args: EventAgrs): void {
        switch (args.bind) {
            ...
            default: G32Log.e(`off ${args.bind} 未知事件类型`)
        }
    }
    ...
    private _onD(args: EventAgrs) {
        ...
        if (this._eventsD.has(args.type)) {
            G32Log.e(`${args.type} 重复注册`);
            return;
        }
        G32SdkSingleton.get(G32SdkDispatcher)?.add(args.type, args.cb!, args.target, args.once);
    ...
    private _offD(args: EventAgrs) {
        ...
        if (this._eventsD.has(args.type)) {
            //事件移除
            G32SdkSingleton.get(G32SdkDispatcher)?.remove(args.type, args.target);
            //删除本地事件
