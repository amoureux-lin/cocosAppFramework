import { BlockInputEvents, Color, instantiate, Node, Prefab, SafeArea, Widget } from "cc";
import { UICallbacks, ViewParams } from "./Defines";
import { DelegateComponent } from "./DelegateComponent";
import { Collection } from "../../Core/Utils/Collection";
import { UIConfig } from "../../Core/Interface/Interface";
...
export class LayerUI extends Node {
    ...
    add(uiid: number, config: UIConfig, params?: any, callbacks?: UICallbacks) {
        ...
        if(params && params.maskColor){
            vp.maskColor = params.maskColor;
            delete params.maskColor;
        } 
        vp.params = params ?? {};
    ...
    protected async load(vp: ViewParams, bundle?: string) {
        // 加载界面资源超时提示
        const timerId = setTimeout(this.onLoadingTimeoutGui, app.config.loadingTimeoutGui);
        ...
        if (vp && vp.node) {
            ...
            bundle = bundle || app.res.defaultBundleName;
            const res = await app.res.loadAsync(bundle, vp.config.prefab, Prefab);
            if (res) {
                vp.node = instantiate(res);
                // 是否启动真机安全区域显示
                if (vp.config.safeArea) vp.node.addComponent(SafeArea);
                vp.node.addComponent(BlockInputEvents)
                // 窗口事件委托
