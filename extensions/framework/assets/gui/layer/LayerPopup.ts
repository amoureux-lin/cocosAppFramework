...
import { BlockInputEvents, Color, EventTouch, Layers, Node, Sprite, tween, UIOpacity } from "cc";
...
export class LayerPopUp extends LayerUI {
    ...
    protected async showUi(vp: ViewParams): Promise<boolean> {
        ...
        if (r) {
            ...
            this.openVacancyRemove(vp.config,vp.maskColor);
    ...
    /** 启动触摸非窗口区域关闭 */
    protected openVacancyRemove(config: UIConfig,maskColor:Color) {
        // 背景半透明遮罩
        if (this.mask == null) {
            ...
            this.mask.getComponent(Sprite).color = maskColor;
        ...
        if (config.mask) {
            ...
            tween(this.mask.getComponent(UIOpacity)).to(0.1,{opacity:255}).start();
        }
    }
