import { Camera, Layers, Node, ResolutionPolicy, SafeArea, Widget, game, screen, view, warn } from "cc";
...
export class LayerManager {
     ...
     * @param root  界面根节点
     */
    public initLayer(root: Node, config: any) {
        if (config == null) {
    ...
    private initScreenAdapter() {
        ...
        if (this.windowAspectRatio > this.designAspectRatio) {
            ...
            app.log.info("适配屏幕高度", "【横屏】");
        }
        else {
            finalW = drs.width;
            finalH = finalW * ws.height / ws.width;
            app.log.info("适配屏幕宽度", "【竖屏】");
        }
    ...
    add(configs: { [key: number]: UIConfig }): void {
        for (let key in configs) {
            if(!this.configs[key]){
                this.setConfig(Number(key),configs[key])
            }
        }
    }
     ...
     * 同步打开一个窗口
     * @param uiid          窗口唯一编号
     * @param uiArgs        窗口参数 （弹窗遮罩颜色：maskColor【可选】）
    ...
    app.gui.open(UIID.Loading, {maskColor:new Color(0,0,0,200), （页面传参）}, uic);
     */
    ...
    openAsyncConfig(config: UIConfig): Promise<number> {
        return new Promise(async (resolve, reject) => {
            ...
            await app.gui.openAsync(uiid, { uiid: uiid });
            resolve(uiid);
        });
    }
